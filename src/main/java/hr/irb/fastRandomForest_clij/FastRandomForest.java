...
public class FastRandomForest
  ...
  public Capabilities getCapabilities(){
    return new hr.irb.fastRandomForest_clij.FastRandomTree().getCapabilities();
  }
  ...
  public void buildClassifier(Instances data) throws Exception{
    ...
    hr.irb.fastRandomForest_clij.FastRandomTree rTree = new hr.irb.fastRandomForest_clij.FastRandomTree();
  ...
  public String translateToOcl(int numClasses, int numFeatures) {
    ...
    System.out.println("num classifiers: " + m_bagger.getClassifiers().length);
    ...
    for (int c = 0; c < numClasses; c++) {
      ...
      oclCode.append(" if (export_probabilities) {\n");
      oclCode.append("  WRITE_dst_IMAGE(dst, POS_dst_INSTANCE(x, y, " + c + ", 0), CONVERT_dst_PIXEL_TYPE(value));\n");
      oclCode.append(" }\n");
    }
    oclCode.append(" int2 pos = (int2)(x, y);\n");
    //oclCode.append(" WRITE_IMAGE_2D(dst, pos, (DTYPE_OUT)class);\n");
    oclCode.append(" if (export_probabilities == 0) {\n");
  ...
  private void appendClassifier(StringBuilder oclCode, FastRandomTree classifier, String spaces) {
    if (classifier.m_Attribute == -1) {
      for (int c = 0; c < classifier.m_ClassProbs.length; c++) {
        if (classifier.m_ClassProbs[c] != 0) {
