...
class FastRfBagging extends RandomizableIteratedSingleClassifierEnhancer
  ...
  public void buildClassifier(Instances data, int numThreads,
    ...
    if (!(m_Classifier instanceof hr.irb.fastRandomForest_clij.FastRandomTree))
    ...
    for (int i = 0; i < m_Classifiers.length; i++) {
      hr.irb.fastRandomForest_clij.FastRandomTree curTree = new hr.irb.fastRandomForest_clij.FastRandomTree();
    ...
    try {
      ...
      for (int treeIdx = 0; treeIdx < m_Classifiers.length; treeIdx++) {
        ...
        if (m_Classifiers[treeIdx] instanceof hr.irb.fastRandomForest_clij.FastRandomTree) {

          hr.irb.fastRandomForest_clij.FastRandomTree aTree = (hr.irb.fastRandomForest_clij.FastRandomTree) m_Classifiers[treeIdx];
          aTree.data = bagData;
  ...
  private double computeOOBError( DataCache data,
    ...
    List<Future<Double>> votes =
      new ArrayList<Future<Double>>(data.numInstances);
    for (int i = 0; i < data.numInstances; i++) {
      hr.irb.fastRandomForest_clij.VotesCollectorDataCache aCollector = new VotesCollectorDataCache(m_Classifiers, i, data, inBag);
      votes.add(threadPool.submit(aCollector));
    }
  ...
  public FastRfBagging() {
    ...
    m_Classifier = new hr.irb.fastRandomForest_clij.FastRandomTree();
  }
