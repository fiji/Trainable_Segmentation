...
public class BalancedRandomForest extends AbstractClassifier implements Randomizable
{
	...
	private ai_clij.BalancedRandomTree[] tree = null;
	...
	public void buildClassifier(final Instances data) throws Exception 
	{
		...
		tree = new ai_clij.BalancedRandomTree[ numTrees ];
		...
		List< Future<ai_clij.BalancedRandomTree> > futures =
            new ArrayList< Future<ai_clij.BalancedRandomTree> >( numTrees );
		...
		try
		{
			for(int i = 0; i < numTrees; i++)
			{
				...
				// Create random tree
				final ai_clij.Splitter splitter =
					new Splitter(new GiniFunction(numFeatures, data.getRandomNumberGenerator( random.nextInt() ) ));

				futures.add(exe.submit(new Callable<ai_clij.BalancedRandomTree>() {
					public ai_clij.BalancedRandomTree call() {
