...
public class FeatureStack3D 
{
		...
		return new Callable<ArrayList<ImagePlus>>()
		{
			public ArrayList<ImagePlus> call()
			{
				...
				for(int ch=0; ch < channels.length; ch++)
				{
					...
					final ImagePlus ip = trainableSegmentation_clij.ImageScience.computeDifferentialImage(sigma, xOrder, yOrder, zOrder, channel);
		...
		return new Callable <ArrayList <ImagePlus> >()
		{
			public ArrayList< ImagePlus >call()
			{
				...
				for(int ch=0; ch < channels.length; ch++)
				{
					...
					final ArrayList<ImagePlus> result = trainableSegmentation_clij.ImageScience.computeHessianImages(sigma, absolute, channel);
		...
		return new Callable<ArrayList< ImagePlus >>()
		{
			public ArrayList< ImagePlus > call()
			{
				...
				for(int ch=0; ch < channels.length; ch++)
				{
					...
					final ImagePlus ip = trainableSegmentation_clij.ImageScience.computeLaplacianImage(sigma, channel);
		...
		return new Callable<ArrayList< ImagePlus >>()
		{
			public ArrayList< ImagePlus > call()
			{
				...
				for(int ch=0; ch < channels.length; ch++)
				{
					...
					final ImagePlus ip = trainableSegmentation_clij.ImageScience.computeEdgesImage( sigma, channel );
	 ...
	 */
	public trainableSegmentation_clij.FeatureStackArray getFeatureStackArray()
	{
		// create empty feature stack array, with space for one stack per slice in the original image 
		trainableSegmentation_clij.FeatureStackArray fsa = new FeatureStackArray(
		...
		{
			trainableSegmentation_clij.FeatureStack fs = new FeatureStack( width, height, colorFeatures );
			fsa.set(fs, i);
		}
