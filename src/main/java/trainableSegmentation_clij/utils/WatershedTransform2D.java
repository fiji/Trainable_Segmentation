...
public class WatershedTransform2D {
			...
			double hMax ) 
	{
	    ...
	    LinkedList<trainableSegmentation_clij.utils.Cursor2D> fifo = new LinkedList<trainableSegmentation_clij.utils.Cursor2D>();
	    ...
	    {	    	
	    	...
	    	for(int pixelIndex = heightIndex1; pixelIndex < pixelList.size(); pixelIndex ++)
	    	{
	    		...
	    		final trainableSegmentation_clij.utils.Cursor2D p = pixelRecord.getCursor();
	    		final int i = p.getX();
	    		final int j = p.getY();
	    		...
	    		neigh.setCursor( p );
	    		for( trainableSegmentation_clij.utils.Cursor2D c : neigh.getNeighbors() )
	    		{       			
	    			int u = c.getX();
	    			int v = c.getY();
	    	...
	    	{
	    		// retrieve point p
	    		final trainableSegmentation_clij.utils.Cursor2D p = fifo.poll();
	    		final int i = p.getX();
	    		final int j = p.getY();
	    	...
	    	for(int pixelIndex = heightIndex2; pixelIndex < pixelList.size(); pixelIndex ++, currentIndex++)
	    	{
	    		...
	    		if ( tabLabels[ i ][ j ] == MASK ) // the pixel is inside a new minimum
	    		{
	    			...
	    			while( !fifo.isEmpty() )
	    	    	{
	    				final trainableSegmentation_clij.utils.Cursor2D p2 = fifo.poll();
