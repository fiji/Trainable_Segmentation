...
public class PixelRecord implements Comparable<PixelRecord>{
	...
	private trainableSegmentation_clij.utils.Cursor2D cursor = null;
	private double value = 0;
	private final static AtomicLong seq = new AtomicLong();
	private final long seqNum;
	
	public PixelRecord(
			final trainableSegmentation_clij.utils.Cursor2D cursor,
			final double value)
	{
		this.cursor = cursor;
		this.value = value;
	...
	public PixelRecord(
			...
			final double value )
	{
		this.cursor = new trainableSegmentation_clij.utils.Cursor2D( x, y );
		this.value = value;
		seqNum = seq.getAndIncrement();
	}
